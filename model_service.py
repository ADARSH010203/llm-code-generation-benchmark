...
async def get_model_response_async(model_name: str, prompt: str, context: Dict[str, Any]):
    ...
    model_mapping = {
        "aya-expanse": "c4ai-aya-expanse-32b",
    ...
    try:
        ...
        response = await acompletion(
            ...
            messages=messages,
            api_key=os.getenv("COHERE_API_KEY") if "llama-4-scout" in model_name else os.getenv("CEREBRAS_API_KEY"),
            max_tokens=2000,
            stream=True
        )
...
async def get_parallel_responses(prompt: str, context: Dict[str, Any]):
    claude_gen = get_model_response_async("llama-4-scout", prompt, context)
    openai_gen = get_model_response_async("aya-expanse", prompt, context)
